...
// Last update Sun Jun 26 23:16:40 2016 guillaume wilmot
//
...
void			Sounds::init()
{
  ...
  for (unsigned int i = 0; i < 1; i++)
    {
      if (!(tmp = Mix_LoadWAV(std::string("./assets/sounds/defeat/" + std::to_string(i) + ".wav").c_str())))
	{
	  std::cerr << "Could not load " << std::string("./assets/sounds/defeat/" + std::to_string(i) + ".wav") << std::endl;
	  throw std::exception();
	}
      _sounds[DEFEAT].push_back(tmp);
    }
